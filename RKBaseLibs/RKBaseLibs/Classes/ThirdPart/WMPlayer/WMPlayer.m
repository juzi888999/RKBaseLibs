...
//控制全屏和播放暂停按钮
@property (nonatomic,strong) EnlargeButton  *fullScreenBtn,*playOrPauseBtn,*lockBtn,*backBtn,*rateBtn;
//进度滑块&声音滑块
@property (nonatomic,strong) RKSlider   *progressSlider,*volumeSlider;
//显示缓冲进度和底部的播放进度
...
-(void)initWMPlayer{
    ...
    //playOrPauseBtn
    self.playOrPauseBtn = [EnlargeButton buttonWithType:UIButtonTypeCustom];
    ...
    //slider
    self.progressSlider = [RKSlider new];
    self.progressSlider.minimumValue = 0.0;
    self.progressSlider.maximumValue = 1.0;
    
//    [self.progressSlider setThumbImage:WMPlayerImage(@"dot")  forState:UIControlStateNormal];
    [self.progressSlider setThumbImage:HPImageForKey(@"progressMax") forState:UIControlStateNormal];
    [self.progressSlider setThumbImage:HPImageForKey(@"progressMax") forState:UIControlStateHighlighted];
    ...
    //fullScreenBtn
    self.fullScreenBtn = [EnlargeButton buttonWithType:UIButtonTypeCustom];
    self.fullScreenBtn.extendInsets = UIEdgeInsetsMake(8, 8, 8, 8);
    ...
    //lockBtn
    self.lockBtn = [EnlargeButton buttonWithType:UIButtonTypeCustom];
    ...
    //backBtn
    self.backBtn = [EnlargeButton buttonWithType:UIButtonTypeCustom];
    ...
    //rateBtn
    self.rateBtn = [EnlargeButton buttonWithType:UIButtonTypeCustom];
...
-(void)addUIControlConstraints{
    ...
    [self.lockBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.equalTo(self.contentView).offset(15+20);
...
-(void)creatWMPlayerAndReadyToPlay{
    ...
    //设置player的参数
    if(self.currentItem){
        self.player = [AVQueuePlayer playerWithPlayerItem:self.currentItem];
    }else{
...
- (void)stratDragSlide:(UISlider *)slider{
    ...
    CMTime time = CMTimeMakeWithSeconds(slider.value, self.currentItem.currentTime.timescale);
    CGFloat nowTime = (CGFloat)CMTimeGetSeconds(time);
