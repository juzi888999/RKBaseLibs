//
//  RKCollectionViewAlignmentFlowLayout.m
//  RKBaseLibs
//
...
#import "RKCollectionViewAlignmentFlowLayout.h"

@interface RKCollectionViewAlignmentFlowLayout(){
    //在居中对齐的时候需要知道这行所有cell的宽度总和
    CGFloat _sumCellWidth ;
}
@end

@implementation RKCollectionViewAlignmentFlowLayout
-(instancetype)init{
    return [self initWithType:RKAlignmentLayoutTypeCenter betweenOfCell:5.0];
}
...
-(instancetype)initWthType:(RKAlignmentLayoutType)layoutType{
    return [self initWithType:layoutType betweenOfCell:5.0];
}
-(instancetype)initWithType:(RKAlignmentLayoutType)cellType betweenOfCell:(CGFloat)betweenOfCell{
    self = [super init];
    if (self){
        self.scrollDirection = UICollectionViewScrollDirectionVertical;
        self.minimumInteritemSpacing = betweenOfCell;
        _betweenOfCell = betweenOfCell;
        _layoutType = cellType;
    }
    return self;
}
...
-(void)setCellFrameWith:(NSMutableArray*)layoutAttributes{
    ...
    switch (_layoutType) {
        case RKAlignmentLayoutTypeLeft:
        ...
        case RKAlignmentLayoutTypeCenter:
        ...
        case RKAlignmentLayoutTypeRight:
