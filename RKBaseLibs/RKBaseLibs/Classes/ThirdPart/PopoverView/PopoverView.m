...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        _borderColor = HPColorForKey(@"#4a4a4a");
        _filteColor = HPColorForKey(@"#4a4a4a");
        _titleColor = HPColorForKey(@"#ffffff");
...
-(CGRect)getViewFrame
{
    ...
    if (frame.size.height > MainScreenHeight-self.showPoint.y) {
        frame.size.height = MainScreenHeight-self.showPoint.y;
    }
    for (NSString *title in self.titleArray) {
        CGFloat width = [title textRectWithFont:TITLE_FONT maxSize:CGSizeMake(300, 100) mode:NSLineBreakByCharWrapping].width;
    ...
    //右间隔最小5x
    if ((frame.origin.x + frame.size.width) > MainScreenWidth-5) {
        frame.origin.x = MainScreenWidth - 5 - frame.size.width;
    }
    return frame;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.backgroundView.backgroundColor = self.filteColor;
    cell.contentView.backgroundColor = self.filteColor;
    UIImageView * selectedBackground = [[UIImageView alloc]initWithImage:[UIImage imageWithColor:self.selectedBackgroundColor cornerRadius:0]];
...
-(void)setFilteColor:(UIColor *)filteColor
{
    _filteColor = filteColor;
    [self setNeedsDisplay];
}
