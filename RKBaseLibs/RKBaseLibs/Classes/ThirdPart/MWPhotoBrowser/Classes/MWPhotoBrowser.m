...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(photoBrowser:didSetControlsHidden:)]) {
        [_delegate photoBrowser:self didSetControlsHidden:hidden];
    }
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
            ...
            if (@available(iOS 11.0, *)) {
                self.activityViewController.excludedActivityTypes = @[UIActivityTypeMarkupAsPDF,UIActivityTypeOpenInIBooks];
            } else {
                if (@available(iOS 9.0, *)) {
                    self.activityViewController.excludedActivityTypes = @[UIActivityTypeOpenInIBooks];
                }
            }
