...
- (void)_performLoadUnderlyingImageAndNotifyWithWebURL:(NSURL *)url {
    @try {
        ...
        if (self.disableCachedImage) {
            options |= SDWebImageRefreshCached;
            [[SDImageCachesManager sharedManager] removeImageForKey:[NSString checkString:url.absoluteString] cacheType:SDImageCacheTypeAll completion:^{
                
            }];
        }
        @weakify(self);
        [[HPFileManager shareManager] getExistLocalImageByPath:url completion:^(UIImage *image) {
            @strongify(self);
            if (self) {
                if (image) {
                    ...
                    _webImageOperation = [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:url options:options progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                        if (expectedSize > 0) {
