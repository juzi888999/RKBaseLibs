...
- (void)endPlay {
    ...
    //重置封面
    self.coverImg = HPPlaceholderImage;
...
/*
 * 加载歌曲
 * @param isNew  YES 加载第一首歌曲 , NO 加载下一首歌曲
 */
...
//        }
//    }else {
        url = [NSURL URLWithString:self.currentSong.playUrl];
//    }
    ...
    _timeObserve = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1.0, 1.0) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        if (self) {
            ...
            if (current) {
                ...
                self.playTime = [NSString stringWithFormat:@"%.f",current];
                self.playDuration = [NSString stringWithFormat:@"%.2f",total];
            }
        }
    }];
...
#pragma mark - 播放分享歌曲
- (void)playSharedSong:(RKAudioEntity *)info {
//    //正在播放该歌曲的情况
